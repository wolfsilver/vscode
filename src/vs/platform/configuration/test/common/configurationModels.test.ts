...
suite('ConfigurationModel', () => {
	...
	test('setValue for a key that has sections and not defined', () => {
		...
		// Hacky way to remove nested null prototypes for comparison
		assert.deepStrictEqual(JSON.parse(JSON.stringify(testObject.contents)), { 'a': { 'b': 1 }, 'b': { 'c': 1 }, 'f': 1 });
...
suite('CustomConfigurationModel', () => {
	...
	test('Test update with empty data', () => {
		...
		assert.deepStrictEqual(testObject.configurationModel.contents, {});
